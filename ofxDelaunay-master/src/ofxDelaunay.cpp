...
//    triangles.resize(3*nv);
    triangles.resize(100 * nv);
    
    // 返回包含所有三角形的三个坐标 的list
	Triangulate( nv, &verticesXYZ[0], &triangles[0], ntri );
	
	//copy triangle data to ofxDelaunayTriangle.
	triangleMesh.clear();
	// 设置ofmesh绘制模式
    triangleMesh.setMode(OF_PRIMITIVE_TRIANGLES); //三角形
//    triangleMesh.setMode(OF_PRIMITIVE_POINTS); //点
//    triangleMesh.setMode(OF_PRIMITIVE_LINES);//线
    triangleMesh.enableColors();
	...
	for (int i = 0; i < nv; i++){
        ...
        //adding TexCoord
        if(count%3 == 0){
            triangleMesh.addTexCoord(ofVec2f(0,0));
        }
        if(count%3 == 1){
            triangleMesh.addTexCoord(ofVec2f(4,0));
        }
        if(count%3 == 2){
            triangleMesh.addTexCoord(ofVec2f(3,0));
        }
        count++;
        
        // 给点添加颜色
//        triangleMesh.addColor(ofColor(255 * vertices[i].x / vertices[i].y,
//                                      255 * vertices[i].y / vertices[i].x,
//                                      255));
        //或者ofFloatcolor(1.,1.,1.)
        triangleMesh.addColor(ofColor(240, 240, 240));
